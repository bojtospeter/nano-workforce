...
import { BaseBranchMustExistError, checkConclusions, classifyMergeability, classifyPrLiveness, coalesceTitle, createPullRequest, ensureBaseBranch, ensurePromotionPr, fetchIssueTitle, fetchPrFiles, fetchPrReviews, isNotAPullRequestError, listPrsForHead, type GhReview, type Mergeability, type PrState } from "./github.ts";
...
test("fetchPrReviews: returns the newest page when the fresh review is beyond page one", async () => {
  ...
  assertEquals(reviews?.map((review) => review.id), [101]);
